...
DISPLAY_STATUS_1 = "SELF-TESTING", "STANDBY", "CAR CONNECTED", "CHARGING",\
                   "CHARGING COMPLETE", "NODE DISABLED", "ERROR"
DISPLAY_STATUS_2 = "Charging Disabled", "Charging Enabled", "State Error"
...
charge_restart = False

cur_user = None
node_num = -1

nodes_tested = range(5)
...
class NodeCan:
    def __init__(self, node_static):
        ...
        self.__current_measured = 0x0000
        self.__voltage_measured = 0x0000
    ...
    def get_current_measured(self):
        return self.__current_measured

    def get_voltage_measured(self):
        return self.__voltage_measured
    ...
    def message_prepare_to_display(self, msg):
        ...
        if node_full_num == node_to_display:
            if msg is None:
                ...
                message_state_field = len(DISPLAY_STATUS_1) - 1
                message_sub_state_field = len(DISPLAY_STATUS_2) - 1
                if len(msg.data) > 0:
                    if msg.data[0] < message_state_field:
                        message_state_field = msg.data[0]
                if len(msg.data) > 1:
                    if msg.data[1] < message_sub_state_field:
                        message_sub_state_field = msg.data[1]
                if show_node_status_admin_mode:
                    self.__message_1_to_display = \
                        DISPLAY_STATUS_1[message_state_field]
                    if message_state_field == 0:            # self-testing
                        self.__message_1_font = font_4
                        self.__message_1_color = color_message_white
                        self.__message_1_blinking = False
                    if message_state_field == 1:            # standby
                        self.__message_1_font = font_5
                        self.__message_1_color = color_message_blue
                        self.__message_1_blinking = False
                    if message_state_field == 2:            # car connected
                        self.__message_1_font = font_5
                        self.__message_1_color = color_message_blue
                        self.__message_1_blinking = False       # True
                    if message_state_field == 3:            # charging
                        self.__message_1_font = font_5
                        self.__message_1_color = color_message_green
                        self.__message_1_blinking = False       # True
                    if message_state_field == 4:            # charging complete
                        self.__message_1_font = font_5a
                        self.__message_1_color = color_message_green
                        self.__message_1_blinking = False
                    if message_state_field == 5:            # node disabled
                        self.__message_1_font = font_5
                        self.__message_1_color = color_message_red
                        self.__message_1_blinking = False
                    if message_state_field == 6:            # error
                        self.__message_1_font = font_5
                        self.__message_1_color = color_message_red
                        self.__message_1_blinking = False   # True
                    self.__message_2_to_display = DISPLAY_STATUS_2[message_sub_state_field]
                    self.__message_2_font = font_4
                    self.__message_2_color = color_message_white
                    self.__message_2_blinking = False
                else:
                    if self.__restart_state:
                        ...
                        self.__message_1_color = color_message_red
                        self.__message_1_blinking = False
                        self.__message_2_to_display = "Connect a Car"
                        self.__message_2_font = font_4
                        self.__message_2_color = color_message_white
                        self.__message_2_blinking = False
                    else:
                        ...
                        if message_state_field == 3:  # charging
                            ...
                            self.__message_1_color = color_message_green
                            self.__message_1_blinking = False       # True
                            self.__message_2_to_display = "Amp = XX.X"
                            self.__message_2_font = font_4
                            self.__message_2_color = color_message_white
                            self.__message_2_blinking = False
                        if message_state_field == 4:  # charging complete
                            ...
                            self.__message_1_color = color_message_green
                            self.__message_1_blinking = False
                            self.__message_2_to_display = "Unplug a Cable"
                            self.__message_2_font = font_4
                            self.__message_2_color = color_message_white
                            self.__message_2_blinking = False
                        if message_state_field == 5:  # node disabled
    ...
    def message_parsing(self, msg):
        if msg is not None:
            ...
            if self.__current_node_active is not None:
                ...
                if (old_state == 0x03 or old_state == 0x04) and state == 0x01:
...
def to_forty_one_admin(event):
    ...
    if line is None:
        ...
        name_setup_power_line_amp.set(line.get_max_amp())
...
def to_fourth_screen(event):
    ...
    if nodes_can is not None:
        current_node = str(nodes_can.get_node_user_selected())
    else:
...
def time_label_on_off(event):
    ...
    if time_label_active:
        ...
        show_service_key_message("TIME Label Hidden")
    else:
        time_label_active = True
        show_service_key_message("TIME Label Exposed")
...
def hundreds_label_on_off(event):
    ...
    if hundreds_label_active:
        hundreds_label_active = False
        show_service_key_message("HUNDREDTHS of a Second Disabled")
    else:
        hundreds_label_active = True
        show_service_key_message("HUNDREDTHS of a Second Activated")
...
def show_node_user_or_admin(event):
    ...
    if show_node_status_admin_mode:
        show_node_status_admin_mode = False
        show_service_key_message("USER Mode Messages activated")
    else:
        show_node_status_admin_mode = True
        show_service_key_message("ADMIN Mode Messages activated")
...
def show_single_or_all_nodes(event):
    ...
    if show_all_nodes_in_admin_mode:
        show_all_nodes_in_admin_mode = False
        show_service_key_message("Display SINGLE charger in Admin mode")
    else:
        show_all_nodes_in_admin_mode = True
        show_service_key_message("Display ALL chargers in Admin mode")
...
def poll_on_off(event):
    ...
    if poll_active:
        poll_active = False
        show_service_key_message("STOP POLLING CAN-Bus")
    else:
        poll_active = True
        show_service_key_message("RESUME POLLING CAN-Bus")
...
def can1_on_off(event):
    ...
    if can1_configured:
        ...
        show_service_key_message("CAN1 Disconnected")
    else:
        can1_configured = True
        show_service_key_message("CAN1 Connected")
...
def nodes_restart(event):
    if nodes_can is not None:
        ...
        show_service_key_message("RESTART Nodes")


# noinspection PyUnusedLocal
def charge_active_restart(event):
    global charge_restart
    charge_restart = True
    show_service_key_message("DISABLE CHARGING while Restart Nodes")
...
def setup_power_line_num_changed(*events):
    ...
    if entered_line is None:
        ...
        name_setup_power_line_amp.set(entered_line.get_max_amp())
...
def init_entries_a_36():
    ...
    if node is None:
        ...
        name_admin_node_can_bus.set(node.get_can_bus_id())
...
label_d_36_50_12 = tk.Label(frame_d_36_50,
                            ...
                            fg="red",
                            bg=color_back_dialog)
label_d_36_50_12.place(relx=0.5, rely=0.375, anchor='c')
...
label_a_36_20 = tk.Label(frame_a_36_2,
                         ...
                         bg=color_back,
                         fg='white')
label_a_36_20.place(relx=0.5, rely=0.3, anchor='c')
...
def to_next_line_admin(event):
    ...
    if line_tmp is not None:
        ...
        name_setup_power_line_amp.set(line_tmp.get_max_amp())
...
label_a_41_1 = tk.Label(frame_a_41_1,
                        ...
                        fg=color_heading,
                        bg='white')
label_a_41_1.place(relx=0.5, rely=0.5, anchor='c')
...
entry_a_411 = tk.Entry(frame_a_41_2,
                       ...
                       bd=4,
                       bg="lightgrey")
entry_a_411.place(relx=0.5, rely=0.1, anchor="c")

label_a_41_212 = tk.Label(frame_a_41_2,
                          text="Amp",
                          font=font_2_bold,
                          bg="white")
label_a_41_212.place(relx=0.66, rely=0.1, anchor='c')
...
label_a_41_224 = tk.Label(frame_a_41_2,
                          ...
                          font=font_6,
                          bg="white")
label_a_41_224.place(relx=0.775, rely=0.34, anchor='c')
...
label_a_41_243 = tk.Label(frame_a_41_2,
                          ...
                          fg=color_back,
                          bg="white")
label_a_41_243.place(relx=0.46, rely=0.71, anchor='c')
...
label_a_41_3 = tk.Label(frame_a_41_3,
                        ...
                        bg=color_back,
                        fg='white')
label_a_41_3.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_42_1 = tk.Label(frame_a_42_1,
                        ...
                        fg=color_heading,
                        bg='white')
label_a_42_1.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_42_224 = tk.Label(frame_a_42_2,
                          ...
                          font=font_6,
                          bg="white")
label_a_42_224.place(relx=0.775, rely=0.37, anchor='c')
...
label_a_42_243 = tk.Label(frame_a_42_2,
                          ...
                          fg=color_back,
                          bg="white")
label_a_42_243.place(relx=0.5, rely=0.77, anchor='c')
...
label_a_42_3 = tk.Label(frame_a_42_3,
                        ...
                        bg=color_back,
                        fg='white')
label_a_42_3.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_43_1 = tk.Label(frame_a_43_1,
                        ...
                        fg=color_heading,
                        bg='white')
label_a_43_1.place(relx=0.5, rely=0.5, anchor='c')
...
entry_a_431.place(relx=0.57, rely=0.25, anchor="c")

label_a_43_212 = tk.Label(frame_a_43_2,
                          text="Amp",
                          font=font_11_bold,
                          bg="white")
label_a_43_212.place(relx=0.69, rely=0.25, anchor='c')
...
label_a_43_3 = tk.Label(frame_a_43_3,
                        ...
                        bg=color_back,
                        fg='white')
label_a_43_3.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_44_1 = tk.Label(frame_a_44_1,
                        ...
                        fg=color_heading,
                        bg='white')
label_a_44_1.place(relx=0.5, rely=0.5, anchor='c')
...
entry_a_441 = tk.Entry(frame_a_44_2,
                       ...
                       bd=4,
                       bg="lightgrey")
entry_a_441.place(relx=0.45, rely=0.25, anchor="c")

label_a_44_212 = tk.Label(frame_a_44_2,
                          text="Enter Amp",
                          font=font_11_bold,
                          bg="white")
label_a_44_212.place(relx=0.63, rely=0.25, anchor='c')
...
label_a_44_233 = tk.Label(frame_a_44_2,
                          ...
                          fg=color_back,
                          bg="white")
label_a_44_233.place(relx=0.36, rely=0.86, anchor='c')

label_a_44_234 = tk.Label(frame_a_44_2,
                          text="then Press",
                          font=font_6_bold,
                          bg="white")
label_a_44_234.place(relx=0.48, rely=0.875, anchor='c')
...
label_a_44_3 = tk.Label(frame_a_44_3,
                        ...
                        bg=color_back,
                        fg='white')
label_a_44_3.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_45_1 = tk.Label(frame_a_45_1,
                        ...
                        fg=color_heading,
                        bg='white')
label_a_45_1.place(relx=0.5, rely=0.5, anchor='c')
...
entry_a_451 = tk.Entry(frame_a_45_2,
                       ...
                       bd=4,
                       bg="lightgrey")
entry_a_451.place(relx=0.45, rely=0.25, anchor="c")

label_a_45_212 = tk.Label(frame_a_45_2,
                          text="Enter Amp",
                          font=font_11_bold,
                          bg="white")
label_a_45_212.place(relx=0.63, rely=0.25, anchor='c')
...
label_a_45_3 = tk.Label(frame_a_45_3,
                        ...
                        bg=color_back,
                        fg='white')
label_a_45_3.place(relx=0.5, rely=0.5, anchor='c')
...
entry_a_46 = tk.Entry(frame_a_46_1,
                      ...
                      bd=4,
                      bg="lightgrey")
entry_a_46.place(relx=0.72, rely=0.25, anchor="c")

label_a_46_12 = tk.Label(frame_a_46_1,
                         text="has been deleted",
                         font=font_4_bold,
                         bg="white")
label_a_46_12.place(relx=0.5, rely=0.55, anchor='c')
...
label_a_46_2 = tk.Label(frame_a_46_2,
                        ...
                        bg=color_back,
                        fg='white')
label_a_46_2.place(relx=0.5, rely=0.5, anchor='c')
...
entry_a_47 = tk.Entry(frame_a_47_1,
                      ...
                      bd=4,
                      bg="lightgrey")
entry_a_47.place(relx=0.72, rely=0.25, anchor="c")

label_a_47_12 = tk.Label(frame_a_47_1,
                         text="has been modified",
                         font=font_4_bold,
                         bg="white")
label_a_47_12.place(relx=0.5, rely=0.55, anchor='c')
...
label_a_47_2 = tk.Label(frame_a_47_2,
                        ...
                        bg=color_back,
                        fg='white')
label_a_47_2.place(relx=0.5, rely=0.5, anchor='c')
...
entry_a_48 = tk.Entry(frame_a_48_1,
                      ...
                      bd=4,
                      bg="lightgrey")
entry_a_48.place(relx=0.72, rely=0.25, anchor="c")

label_a_48_12 = tk.Label(frame_a_48_1,
                         text="has been added",
                         font=font_4_bold,
                         bg="white")
label_a_48_12.place(relx=0.5, rely=0.55, anchor='c')
...
label_a_48_2 = tk.Label(frame_a_48_2,
                        ...
                        bg=color_back,
                        fg='white')
label_a_48_2.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_8_1_1 = tk.Label(frame_a_8_1_1,
                         ...
                         font=font_a_8_1,
                         bg="white")
label_a_8_1_1.place(relx=0.5, rely=0.15, anchor="c")
...
entry_a_8_1_1.place(relx=0.5, rely=0.35, anchor="c")

label_a_8_1_2 = tk.Label(frame_a_8_1_1,
                         text="Enter your new PIN to confirm",
                         font=font_a_8_1,
                         bg="white")
label_a_8_1_2.place(relx=0.5, rely=0.55, anchor="c")
...
entry_a_8_1_2.place(relx=0.5, rely=0.75, anchor="c")
...
label_a_8_1_2_1 = tk.Label(frame_a_8_1_2,
                           ...
                           bg=color_back,
                           fg='white')
label_a_8_1_2_1.place(relx=0.5, rely=0.3, anchor='c')
...
label_a_8_1_2_2 = tk.Label(frame_a_8_1_2,
                           ...
                           bg=color_back,
                           fg='white')
label_a_8_1_2_2.place(relx=0.5, rely=0.7, anchor='c')
...
label_a_8_2_11 = tk.Label(frame_a_8_2_1,
                          ...
                          font=font_4_bold,
                          bg="white")
label_a_8_2_11.place(relx=0.5, rely=0.2, anchor='c')

label_a_8_2_12 = tk.Label(frame_a_8_2_1,
                          text='has been successfully\nchanged',
                          font=font_4_bold,
                          bg="white")
label_a_8_2_12.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_8_2_21 = tk.Label(frame_a_8_2_2,
                          ...
                          bg=color_back,
                          fg='white')
label_a_8_2_21.place(relx=0.5, rely=0.3, anchor='c')
...
label_a_8_2_22 = tk.Label(frame_a_8_2_2,
                          ...
                          bg=color_back,
                          fg='white')
label_a_8_2_22.place(relx=0.5, rely=0.7, anchor='c')
...
label_a_8_3_1 = tk.Label(frame_a_8_3_1,
                         ...
                         font=font_a_8_1,
                         bg="white")
label_a_8_3_1.place(relx=0.5, rely=0.15, anchor="c")
...
entry_a_8_3_1.place(relx=0.5, rely=0.35, anchor="c")

label_a_8_3_2 = tk.Label(frame_a_8_3_1,
                         text="Enter your new Password to confirm",
                         font=font_a_8_1,
                         bg="white")
label_a_8_3_2.place(relx=0.5, rely=0.55, anchor="c")
...
entry_a_8_3_2.place(relx=0.5, rely=0.75, anchor="c")
...
label_a_8_3_2_1 = tk.Label(frame_a_8_3_2,
                           ...
                           bg=color_back,
                           fg='white')
label_a_8_3_2_1.place(relx=0.5, rely=0.3, anchor='c')
...
label_a_8_3_2_2 = tk.Label(frame_a_8_3_2,
                           ...
                           bg=color_back,
                           fg='white')
label_a_8_3_2_2.place(relx=0.5, rely=0.7, anchor='c')
...
label_a_8_4_11 = tk.Label(frame_a_8_4_1,
                          ...
                          font=font_4_bold,
                          bg="white")
label_a_8_4_11.place(relx=0.5, rely=0.2, anchor='c')

label_a_8_4_12 = tk.Label(frame_a_8_4_1,
                          text='has been successfully\nchanged',
                          font=font_4_bold,
                          bg="white")
label_a_8_4_12.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_8_4_21 = tk.Label(frame_a_8_4_2,
                          ...
                          bg=color_back,
                          fg='white')
label_a_8_4_21.place(relx=0.5, rely=0.5, anchor='c')
...
label_a_11_1 = tk.Label(frame_a_11_1,
                        ...
                        fg=color_heading,
                        bg='white')
label_a_11_1.place(relx=0.5, rely=0.5, anchor='c')
...
root.bind("<Shift-F8>", charge_active_restart)
